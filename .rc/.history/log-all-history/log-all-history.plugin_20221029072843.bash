...
if ! [[ $PROMPT_COMMAND =~ detailed_hist ]] ; then
	if [[ -z ${PROMPT_COMMAND} ]]; then
		PROMPT_COMMAND="detailed_hist"
	else
		PROMPT_COMMAND="detailed_hist;${PROMPT_COMMAND}"
	fi
fi
...
hdgrep() {
	WHERE="$LOG_DESTINATION/$(pwd)"
	grep -Rh $@ $WHERE
}

hgrep() {
	WHERE="$LOG_DESTINATION"
	grep -Rh $@ $WHERE
}
